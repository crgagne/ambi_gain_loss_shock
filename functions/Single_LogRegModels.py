...
def fit_model_singRL(trial_table,params,combined=False,split_gain_loss=False,return_gain_or_loss='gain',zscore=True):
   ...
   if 'prob_r' in params:
	if combined:
		X['prob_right_gain'] = trial_table['prob_o_r_bayes']
		X['prob_left_gain'] = trial_table['prob_o_l_bayes']
		X.loc[trial_table['gain_or_loss_trial']=='loss','prob_right_gain']=0
		X.loc[trial_table['gain_or_loss_trial']=='loss','prob_left_gain']=0
		X['prob_right_loss'] = trial_table['prob_o_r_bayes']
		X['prob_left_loss'] = trial_table['prob_o_l_bayes']
		X.loc[trial_table['gain_or_loss_trial']=='gain','prob_right_loss']=0
		X.loc[trial_table['gain_or_loss_trial']=='gain','prob_left_loss']=0
	else:
		X['prob_right'] = trial_table['prob_o_r_bayes']
		X['prob_left'] = trial_table['prob_o_l_bayes']
   # prob diff
   if 'prob_diff_rl' in params:
   	if combined:
		X['prob_diff_rl_gain'] = trial_table['prob_o_r_bayes']-trial_table['prob_o_l_bayes']
		X.loc[trial_table['gain_or_loss_trial']=='loss','prob_diff_rl_gain']=0
		X['prob_diff_rl_loss'] = trial_table['prob_o_r_bayes']-trial_table['prob_o_l_bayes']
		X.loc[trial_table['gain_or_loss_trial']=='gain','prob_diff_rl_loss']=0
   	else:
		X['prob_diff_rl'] = trial_table['prob_o_r_bayes']-trial_table['prob_o_l_bayes']

   # ev diff
   if 'ev_diff' in params:
	if combined:
		X['ev_diff_rl_gain'] = trial_table['prob_o_r_bayes']*trial_table['mag_right']-trial_table['prob_o_l_bayes']*trial_table['mag_left']
		X['ev_diff_rl_loss'] = trial_table['prob_o_r_bayes']*trial_table['mag_right']-trial_table['prob_o_l_bayes']*trial_table['mag_left']
		X.loc[trial_table['gain_or_loss_trial']=='loss','ev_diff_rl_gain']=0
		X.loc[trial_table['gain_or_loss_trial']=='gain','ev_diff_rl_loss']=0
	else:
		X['ev_diff_rl'] = trial_table['prob_o_r_bayes']*trial_table['mag_right']-trial_table['prob_o_l_bayes']*trial_table['mag_left']
   ...
   # create unambiguous probability
       ...
       #Tracer()()
    #   if combined:
    #       X['inter_unambig_prob_cont_ambig_gain']=trial_table['prob_o_unambig']*X['sqrt_prop_revealed_diff_rl_gain'] # the difference is important to make it so it's right v left choice
    #       X['inter_unambig_prob_cont_ambig_loss']=trial_table['prob_o_unambig']*X['sqrt_prop_revealed_diff_rl_loss']
  ...
  #       if combined:
#           X['inter_unambig_ev_cont_ambig_gain']=trial_table['prob_o_unambig']*trial_table['mag_unambig']*X['sqrt_prop_revealed_diff_rl_gain']
#           X['inter_unambig_ev_cont_ambig_loss']=trial_table['prob_o_unambig']*trial_table['mag_unambig']*X['sqrt_prop_revealed_diff_rl_loss']
...
#           X.loc[trial_table['gain_or_loss_trial']=='gain','inter_unambig_ev_cont_ambig_loss']=0
   ...
   if 'inter_unambig_prob_cat_ambig' in params:
       if combined:
          prob_unambig = trial_table['ambig_r']*trial_table['prob_o_l']+trial_table['ambig_l']*trial_table['prob_o_r'] #this will be first or second prob, not the sum.
   ...
   if 'inter_ambig_prob_cat_ambig' in params:
       if combined:
          prob_ambig = trial_table['ambig_r']*trial_table['prob_o_r_bayes']+trial_table['ambig_l']*trial_table['prob_o_l_bayes'] #this will be first or second prob, not the sum.
   ...
   if 'inter_cat_abs_prob' in params:
        if combined:
            abs_prob = trial_table['prob_o_r_bayes']+trial_table['prob_o_l_bayes']
...
#        if combined:
#          X['inter_unambig_ev_cat_ambig_gain']=trial_table['prob_o_unambig']*trial_table['mag_unambig']*X['ambig_present_diff_rl_gain']
#          X['inter_unambig_ev_cat_ambig_loss']=trial_table['prob_o_unambig']*trial_table['mag_unambig']*X['ambig_present_diff_rl_loss']
